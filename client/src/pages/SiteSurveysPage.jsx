...
import { Map, CheckCircle2, Search, Filter, Calendar, X, Eye, FileText, Zap, Home } from 'lucide-react';
...
export default function SiteSurveysPage() {
  ...
  // For each project, there might or might not be a survey yet
  const surveyList = projects.map(proj => {
    const srv = surveys.find(s => s.solarProjectId === proj.id);
    return {
      projectId: proj.id,
      projectName: proj.title || proj.clientName,
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              ...
              {surveyList.length === 0 && (
                <tr>
                  <td colSpan="7" className="p-8 text-center text-muted font-bold text-sm">No site surveys found.</td>
                </tr>
              )}
            </tbody>
          </table>
